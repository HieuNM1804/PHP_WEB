...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['main_image']) && $_FILES['main_image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!in_array($_FILES['main_image']['type'], $allowed)) {
            ...
            // Xóa ảnh cũ
            if ($product['main_image'] && file_exists($uploadDir . $product['main_image'])) {
    ...
    if (empty($errors)) {
        $stmt = $pdo->prepare("UPDATE products SET product_name=?, category_id=?, supplier_id=?, unit_price=?, old_price=?, stock_quantity=?, main_image=?, description=?, status=? WHERE product_id=?");
        $stmt->execute([$name, $catId ?: null, $supId ?: null, $price, $oldPrice, $stock, $imageName, $desc, $status, $id]);
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <div class="form-grid">
            <div class="form-group full-width">
                ...
                <input type="text" id="product_name" name="product_name" class="form-control" value="<?= e($product['product_name']) ?>" required>
            </div>
            ...
            <div class="form-group">
                ...
                <select id="supplier_id" name="supplier_id" class="form-control">
                    ...
                    <?php foreach ($suppliers as $sup): ?>
                        <option value="<?= $sup['supplier_id'] ?>" <?= ($product['supplier_id'] == $sup['supplier_id']) ? 'selected' : '' ?>><?= e($sup['supplier_name']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="number" id="unit_price" name="unit_price" class="form-control" step="1000" min="0" value="<?= $product['unit_price'] ?>" required>
            </div>

            <div class="form-group">
                <label for="old_price">Giá cũ (nếu có)</label>
                <input type="number" id="old_price" name="old_price" class="form-control" step="1000" min="0" value="<?= $product['old_price'] ?? '' ?>">
            </div>

            <div class="form-group">
                <label for="stock_quantity">Tồn kho</label>
                <input type="number" id="stock_quantity" name="stock_quantity" class="form-control" min="0" value="<?= $product['stock_quantity'] ?>">
            </div>
            ...
            <div class="form-group full-width">
                ...
                <?php if ($product['main_image'] && file_exists(__DIR__ . '/../../assets/uploads/' . $product['main_image'])): ?>
                    <img src="<?= baseUrl('assets/uploads/' . $product['main_image']) ?>" style="max-height:120px; width:auto; object-fit:contain; border-radius:8px; margin-bottom:10px; display:block;" alt="Current">
                <?php endif; ?>
            ...
            <div class="form-group full-width">
                ...
                <textarea id="description" name="description" class="form-control" rows="4"><?= e($product['description']) ?></textarea>
            </div>
        </div>
